...
flags.DEFINE_integer('hidden_l2', 50, 'Number of hidden neurons in layer 2.')
...
with tf.device(device):
    # Input
    x = tf.placeholder(tf.float32, shape=[None, 48], name='x-input')
    # Output
    y_ = tf.placeholder(tf.float32, shape=[None, 4], name='desired-output')

    # Hidden layer 1 weights and bias
    W_1 = tf.Variable(tf.random_uniform([48, settings.hidden_l1]), name='weights-1')
...
summary_dir = '../../logs/gridworld-nn-episodes{}-hidden1_{}-hidden2_{}-lr{}-{}/'.format(settings.episodes, 
    settings.hidden_l1, settings.hidden_l2, settings.learning_rate, settings.optimizer)
...
while settings.episodes > episode:
    ...
    while not terminal and step < step_limit: 
        ...
        q_values = sess.run(y, feed_dict={x: state.reshape(1,48)})
        ...
        q_values_new = sess.run(y, feed_dict={x: new_state.reshape(1,48)})
        ...
        _, loss = sess.run([train_step, obj_function],
                    feed_dict={x: state.reshape(1,48),
                            y_: q_values})

        # Calculate accuracy
        acc = sess.run(accuracy, feed_dict={x: state.reshape(1,48), 
                                            y_: q_values})
