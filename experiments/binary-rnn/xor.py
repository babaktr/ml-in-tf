...
with tf.name_scope('loss') as scope:
    obj_function = tf.multiply(0.5, tf.reduce_sum(tf.sub(y, y_) * tf.sub(y, y_)))
...
correct_prediction = tf.subtract(1.0, tf.abs(tf.subtract(y_,y)))
...
for i in range (settings.batches): 
    ...
    acc = sess.run(accuracy, feed_dict={x: training_inputs, 
                                        y_: training_outputs})
